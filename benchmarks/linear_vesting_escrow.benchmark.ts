import { type AccountWallet, type ContractFunctionInteraction, type PXE, getContractClassFromArtifact, Fr, GrumpkinScalar } from '@aztec/aztec.js';
...
import { AMOUNT, deployEscrowWithPublicKeysAndSalt, deployLinearVestingEscrow, deployTokenWithMinter, setupPXE, grumpkinScalarToFr } from '../src/ts/utils.js';
...
export default class TokenContractBenchmark extends Benchmark {
  ...
  async setup(): Promise<LinearVestingEscrowBenchmarkContext> {
    ...
    const deployedLinearVestingEscrow = await deployLinearVestingEscrow(deployer, escrowClassId,);
    const linearVestingEscrowContract = await LinearVestingEscrowLogicContract.at(deployedLinearVestingEscrow.address, deployer);

    const escrowSk = Fr.ONE.add(Fr.ONE);
    const escrowKeys = await deriveKeys(escrowSk);
    const escrowSalt = new Fr(linearVestingEscrowContract.instance.address.toBigInt());
    const escrowContract = (await deployEscrowWithPublicKeysAndSalt(escrowKeys.publicKeys, deployer, escrowSalt,)) as EscrowContract;
    const partialAddressEscrow = await escrowContract.partialAddress;
    await pxe.registerAccount(escrowSk, partialAddressEscrow);

    const tokenContract = (await deployTokenWithMinter(deployer, {})) as TokenContract;
    await tokenContract
        .withWallet(deployer)
    ...
    return { pxe, deployer, accounts, linearVestingEscrowContract, escrowContract, tokenContract, secretKeys, start, duration };
  }

  /**
   * Returns the list of TokenContract methods to be benchmarked.
   */
  getMethods(context: LinearVestingEscrowBenchmarkContext): ContractFunctionInteraction[] {
    const { linearVestingEscrowContract, accounts, escrowContract, tokenContract, secretKeys, start, duration } = context;
    const [alice, bob] = accounts;
    const methods: ContractFunctionInteraction[] = [
      // Create linear vesting escrow
      linearVestingEscrowContract.withWallet(alice).methods.create_linear_vesting_escrow(escrowContract.instance.address, bob.getAddress(), tokenContract.instance.address, start, duration, AMOUNT, secretKeys[0], secretKeys[1], secretKeys[2], secretKeys[3]),
      // Claim linear vesting escrow
      linearVestingEscrowContract.withWallet(bob).methods.claim(escrowContract.instance.address),
    ];
